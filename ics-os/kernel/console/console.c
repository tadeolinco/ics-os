...
void getstring(char *buf, DEX32_DDL_INFO *dev)
{
    ...
    do
    {
        ...
        if (c == '\t')
        {
            ...
            while (u != 0)
            {                // counts number of words in buf and
            ...
            else if (input_len > 1)
            {
                ...
                for (int index = 0; index < totalfiles; ++index)
                {
                    ...
                    if (strlen(buffer[index].name) > buf_len)
                    {
                        ...
                        if (strcmp(last_input, sub_buf) == 0)
                        {
                            while (buf_len < strlen(fname))
                            { // why wont for loops work
...
{
    int day = (((year % 100) / 4) + 1 + get_month_code(month, year) + get_century_code(year) + (year % 100)) % 7;
